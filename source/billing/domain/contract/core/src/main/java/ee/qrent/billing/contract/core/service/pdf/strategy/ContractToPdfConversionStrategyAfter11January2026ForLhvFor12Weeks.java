...
public class ContractToPdfConversionStrategyAfter11January2026ForLhvFor12Weeks
    extends AbstractContractToPdfConversionStrategy {

  public ContractToPdfConversionStrategyAfter11January2026ForLhvFor12Weeks(
  ...
  public boolean canApply(final ContractPdfModel model) {
    ...
    return isContractAfterNew1ContractDateAndLhv(model)
 ...
 //   addLhvChapterIfNecessary(model, pdfDocument);
    ...
    chapter1.addCell(
        getSubChapterText(
            "Allkirjastades käesoleva lepingu kinnitab Rentnik, et ta sõlmib lepingu füüsilise isikuna, kes tegutseb ettevõtjana ning teostab oma " +
                    "ettevõtlust LHV ettevõtluskonto kaudu. Rentnik kasutab renditud sõidukit üksnes ettevõtlustulu teenimiseks tasulise sõiduteenuse " +
                    "osutamisel või muul majandus- või kutsetegevusel, mis on otseselt seotud tema ettevõtlusega, ega kasuta sõidukit isiklikes ega olmelistes eesmärkides."));
    chapter1.addCell(getSubChapterNumber("1.1.1"));
    chapter1.addCell(
        getSubChapterText(
            "Rentnik kinnitab, et ta ei tegutse tarbijana VÕS § 1 lg 5 ega § 143 tähenduses ning pooled kinnitavad üheselt, et käesolev leping ei ole " +
                    "tarbijaleping ega kuulu tarbijalepingutele kohalduva regulatsiooni alla."));
    ...
    chapter1.addCell(
        getSubChapterText(
            "Kõik käesoleva lepinguga seotud või sellele lisatud dokumendid, mis on loetletud käesoleva lepingu punktis 1.1.1, on " +
                    "käesoleva lepingu lahutamatu osa ning on pooltele siduvad kogu rendilepingu kehtivusaja jooksul."));

    chapter1.addCell(getSubChapterNumber("1.2"));
    chapter1.addCell(
        getSubChapterText(
            "Käesoleva lepingulisa “Tüüptingimused” nr. 081225 lepingulisa on koostatud ja allkirjastatud 08.12.2025 (Signature Timestamp UTC - 08.12.2025 18:41:05 +00:00) ning on saadaval järgmisel lingil:"
                + "https://drive.google.com/file/d/1w_OYDVHJ_WP0xF4iCJf7GCezgap6vOVM/view?usp=sharing"));
    ...
    chapter1.addCell(
            getSubChapterText(
                    "Käesoleva lepingulisa eestikeelne tekst on saadaval järgmisel lingil: "  // Ссылка на страничку на сайте с эстонским текстом из типовых условий.
                            + "https://qrent.ee/tuuptingimused/tt_lhv_pdf_est"));
    ...
    chapter1.addCell(
            getSubChapterText(
                    "Käesoleva lepingulisa venekeelne (по-русски) tekst on saadaval järgmisel lingil: " //  Ссылка на страничку на сайте с русским переводом.
                            + "https://qrent.ee/tuuptingimused/tt_lhv_pdf_rus"));
    ...
    chapter1.addCell(
            getSubChapterText(
                    "Käesoleva lepingulisa inglisekeelne (in English) tekst on saadaval järgmisel lingil: " //  Ссылка на страничку на сайте с английским переводом.
                            + "https://qrent.ee/tuuptingimused/tt_lhv_pdf_eng"));
    ...
    chapter1.addCell(
            getSubChapterText(
                    "Käesoleva lepingu ja “Tüüptingimused” lepingulisa tingimuste ebaõige täitmise või täitmata jätmise korral kannavad " +
                            "pooled vastutust käesoleva lepingu, “Tüüptingimused” lepingulisa ja kehtivate õigusnormide järgi."));

    chapter1.addCell(getSubChapterNumber("1.11"));
    chapter1.addCell(
        getSubChapterText(
            "Rendileandja ja Rentnik sõlmivad käesoleva lepingu oma majandustegevuse raames, mille alusel on Rentnikul õigus võtta renti igat vaba autot Rendileandja autopargist " +
                    "oma tulu teenimise eesmärgil, s.o taksoteenuste osutamiseks. Vastavalt käesolevale rendilepingule kohustub Rendileandja andma Rentnikule kasutada käesolevas " +
                    "lepingus, selle “Tüüptingimused” lepingulisas ning üleandmise-vastuvõtmise aktis määratud rendieseme (vaba auto Rendileandja autopargist, kui see on olemas)." +
                    " Selle eest on Rentnik kohustatud tasuma Rendileandjale renditasu (rent) kogu rendiperioodi eest. Rentnik kinnitab, et renditeenuse kasutamine toimub " +
                    "üksnes ettevõtlustulu teenimiseks LHV ettevõtluskonto kaudu."));
    ...
    chapter1.addCell(
        getSubChapterText(
            "Oma allkirjaga kinnitab Rentnik, et on ta teadlik ja nõustub, et iga renditud auto täispika kalendrinädala (Esmaspäevast – Pühhapäevani kaasarvatud) " +
                    "rendimaksumus on määratud vastavas üleandmise-vastuvõtmise aktis ning et tuleb nimetatud tasu tasuda õigeaegselt ja täies ulatuses - jooksva rendinädala " +
                    "Teisipäeva kella 16:00’ni. Hilinenud makse loetakse oluliseks lepingurikkumiseks käesoleva lepingu mõttes. Maksekorralduses märgitud selgitus ei mõjuta " +
                    "ettemaksu arvestamist vastavalt Tüüptingimuste p 2.1.5."));

    chapter1.addCell(getSubChapterNumber("1.12.1"));
    chapter1.addCell(
        getSubChapterText(
            "Rentnik kinnitab nõusolekut osaleda Rendileandja boonuskampaaniates, mille üldtingimused, arvestamise kord, eeltingimused, muutmise ja lõpetamise reeglid on sätestatud " +
                    "Tüüptingimuste peatükis XV. Rentnik nõustub, et boonuste andmine ei ole Rendileandja kohustus ning kampaaniate kohaldamine toimub üksnes Tüüptingimustes määratud alustel."));
    ...
    chapter2.addCell(
        getSubChapterText(
            "Käesoleva lepingulisa “TSK Tingimused” nr. 08122025 lepingulisa on koostatud ja allkirjastatud 08.12.2025 (Signature Timestamp UTC - 08.12.2025 18:45:25 +00:00) ning on saadaval järgmisel lingil:"
                + "https://drive.google.com/file/d/1ITSMo7MKpqXXTekilq86opRpooF70aGT/view?usp=sharing "));

    chapter2.addCell(getSubChapterNumber("2.1.1"));
    chapter2.addCell(
            getSubChapterText(
                    "Käesoleva lepingulisa eestikeelne tekst on saadaval järgmisel lingil: "  //  Ссылка на страничку на сайте с эстонским текстом из типовых условий.
                            + "https://qrent.ee/tsk/tsk_lhv_pdf_est"));

    chapter2.addCell(getSubChapterNumber("2.1.2"));
    chapter2.addCell(
            getSubChapterText(
                    "Käesoleva lepingulisa venekeelne (по-русски) tekst on saadaval järgmisel lingil: "  //  Ссылка на страничку на сайте с русским переводом.
                            + "https://qrent.ee/tsk/tsk_lhv_pdf_rus"));
    ...
    chapter2.addCell(
            getSubChapterText(
                    "Käesoleva lepingulisa inglisekeelne (in English) tekst on saadaval järgmisel lingil:" //  ССсылка на страничку на сайте с английским переводом.
                            + "https://qrent.ee/tsk/tsk_lhv_pdf_eng"));
    ...
    chapter5.addCell(
            ...
            +" https://qrent.ee/koostleping_lhv_pdf_rus"));        //    Ссылка на страничку на сайте с русским переводом.\n"));

    chapter5.addCell(getSubChapterNumber("5.6.2"));
    chapter5.addCell(
            getSubChapterText(
                    "Käesoleva lepingu inglisekeelne (in English) tekst on saadaval järgmisel lingil:"
            +" https://qrent.ee/koostleping_lhv_pdf_eng"));     //    Ссылка на страничку на сайте с английским переводом.\n"));
    ...
    final var body16acell2 =
            new Cell(
                    new Paragraph(
                            "\"Uus Juht\"  Lisa lepingule nr.  "
                                    + contractNumber
                                    + "\nKäesolev lisa on individuaalne kokkulepe ja selle tingimused on ülimuslikud Tüüptingimuste XV peatüki ees. Rendileandja ei või muuta käesoleva lisa tingimusi ühepoolselt.\n\n"
    ...
    chapter16.addCell(body16acell3);
    chapter16.addCell(
            getSubChapterText(
                    format(
                            "Käesoleva lepingulisaga reguleerivad pooled Rendileandja sõiduki kasutamise eest makstava renditasu boonustingimusi esimese 12 (kaheteistkümne) " +
                                    "täiskalendrinädala jooksul vastavalt rendilepingule nr. %s ja boonuskampaaniale \"Uus Juht\".",
                            contractNumber)));
    ...
    campaniachapter.addCell(
            getSubChapterText(
                    "Boonusprogramm kehtib ainult uuele Rentnikule, kes sõlmib rendilepingu "
                     +  model.getQFirmName()
                     + "-ga esmakordselt ega ole varem kasutanud Rendileandja rendiautosid."));
    ...
    campaniachapter.addCell(
            getSubChapterText(
                    "ädalaboonuse arvestamise aluseks võetakse rendiauto üleandmise-vastuvõtmise aktis märgitud kehtiv nädalarendi hind."));

    campaniachapter.addCell(getSubChapterNumber("2.2"));
    campaniachapter.addCell(
            getSubChapterText(
                    """
                                Rentnik saab rendihinna alusel rahalise boonuse järgmiselt (täpsem boonusgraafik on lisatud käesolevale lisale):
                                – 25% boonus 1., 4., 7. ja 9. nädala eest;
                                – 10% boonus 2., 3., 5., 6., 8., 10., 11. ja 12. nädala eest.\s"""));

    campaniachapter.addCell(getSubChapterNumber("2.3"));
    campaniachapter.addCell(
            getSubChapterText(
                    "Boonus arvestatakse saldole ainult juhul, kui eelneva nädala renditasu ja kõik võlakohustused on tasutud korrektselt ja tähtaegselt (vt p. 2.4 ja 2.5)"));
    ...
    campaniachapter.addCell(
            getSubChapterText(
                    """
                                ...
                                – kas ülekandega Rendileandja pangakontole (või muule määratud kontole), märkides selgituseks „autorent + auto number“,
                                – või sularahas Rendileandja kontoris aadressil Lasnamäe 30a, Tallinn, hiljemalt teisipäevaks kell 16:00 jooksva nädala eest.\s"""));
    ...
    campaniachapter.addCell(getSubChapterNumber("2.7"));
    campaniachapter.addCell(
            getSubChapterText(
                    "Boonust arvestatakse üksnes juhul, kui Rentnik on täitnud kõik maksekohustused vastavalt Tüüptingimuste punktile 12.14. " +
                            "Boonust ei arvestata ega kasutata enne, kui punktides 12.14.1–12.14.4 nimetatud summad on täielikult tasutud."));
    ...
    campaniachapter.addCell(
            getSubChapterText(
                    "Kui Rentnik rikub kampaanias sätestatud tingimusi või lõpetab rendilepingu nr.  "
                            + contractNumber
                            + " ennetähtaegselt enne miinimumperioodi (määratud koostöölepingu punktides 1.13 ja 1.14), on Rentnik kohustatud tasuma ühekordse leppetrahvi summas 432 eurot."));

    campaniachapter.addCell(getSubChapterNumber("3.2"));
    campaniachapter.addCell(
            getSubChapterText(
                    "Käesolev summa vastab 12 nädala jooksul määratud boonuste kogusummale ja kohaldub sõltumata Rentniku muudest lepingulistest kohustustest. " +
                            "Käesolev leppetrahv ei asenda ega tühista muid lepingus, Tüüptingimustes, hinnakirjas või lisades sätestatud sanktsioone. " +
                            "Kõik muud lepingujärgsed summad, mis kuuluvad tasumisele, võivad samuti olla sisse nõutud täies ulatuses."));

    campaniachapter.addCell(getSubChapterNumber("3.3"));
    campaniachapter.addCell(
            getSubChapterText(
                    "Käesoleva lisa boonustingimused kehtivad üksnes juhul, kui Rentnik kasutab rendiperioodi jooksul sõidukit, mille nädalarendi baashind vastab " +
                            "üleandmise-vastuvõtmise aktis märgitud baashinnale, mis kehtis rendilepingu sõlmimise hetkel. Kui Rentnik vahetab sõiduki teise nädalarendi " +
                            "baashinnaga sõiduki vastu või kui nädalarendi baashind muutub uue üleandmise-vastuvõtmise akti alusel, ei kohaldu käesoleva lisa boonustingimused."));

    campaniachapter.addCell(getSubChapterNumber("3.4"));
    campaniachapter.addCell(
            getSubChapterText(
                    "Juhul kui Rentnik kasutab käesoleva lisa kehtivusajal mis tahes põhjusel sõidukit, mille nädalarendi baashind ei vasta rendilepingu sõlmimise hetkel " +
                            "üleandmise-vastuvõtmise aktis märgitud algsele baashinnale, ei arvestata vastava nädala eest käesoleva kampaania 25% ega 10% boonust." +
                            " Kampaania „Uus Juht“ ei lõpe, kuid boonuse arvestamine peatub automaatselt nende nädalate osas, mil Rentnik kasutab teistsuguse baashinnaga sõidukit. " +
                            "Sellistel nädalatel rakendatakse Rentnikule renditasu arvestamist vastavalt kasutatava sõiduki tegelikule baashinnale ning Rendileandjal " +
                            "on õigus kohaldada kampaania „Usaldusväärne Partner“ 5% või 10% boonust vastavalt selle kampaania kehtivatele tingimustele."));

    campaniachapter.addCell(getSubChapterNumber("3.5"));
    campaniachapter.addCell(
            getSubChapterText(
                    "Pärast kampaania „Uus Juht“ kehtivusaja lõppemist on Rendileandjal õigus, kuid mitte kohustus, rakendada Rentnikule automaatselt kampaaniat " +
                            "„Usaldusväärne Partner“ vastavalt selle kampaania kehtivatele tingimustele. Kampaania rakendamine toimub Rendileandja äranägemisel ning ei eelda eraldi kokkulepet. "));
