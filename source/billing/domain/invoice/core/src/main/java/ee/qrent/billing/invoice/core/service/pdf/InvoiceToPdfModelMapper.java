...
public class InvoiceToPdfModelMapper {
  ...
  private static final BigDecimal VAT_RATE_2025 = BigDecimal.valueOf(24);
  private static final LocalDate VAT_CHANGE_DATE = LocalDate.of(2025, 7, 1);


// TODO VAT Rate Set value 24 from 01.07.2025
  ...
  public InvoicePdfModel getPdfModel(final Invoice invoice) {
    ...
    //---------
//    final var totalWithFee = getTotalAmountWithFeeFromPreviousInvoice(driverId, currentQWeekId);
//    final var previousQWeek = qWeekQuery.getOneBeforeById(currentQWeekId);
//    final var previousInvoice = invoiceLoadPort.loadByQWeekIdAndDriverId(previousQWeek.getId(), driverId);
//    final var previousWeekFee = previousInvoice != null ? previousInvoice.getCurrentWeekFee() : BigDecimal.ZERO;
...
//--------------
    ...
    //    .block2A(qRound(block2AValue))
        .block2A(qRound(invoice.getPreviousWeekAllPositiveTxSum()))
    //    .block2B(qRound(invoice.getPreviousWeekPositiveTxSum()))
        .block2B(qRound(invoice.getPreviousWeekAllNegativeTxSum().negate()))
        .block2C(qRound(block2C))
        .build();
  }

  private BigDecimal getVatRate(final Invoice invoice, final QWeekResponse invoiceWeek) {
    if (invoice.withVat()) {
      if (!invoiceWeek.getStart().isBefore(VAT_CHANGE_DATE)) {
        return VAT_RATE_2025;
      }
  ...
  private BigDecimal getTotalAmountWithFeeFromPreviousInvoice(
    ...
    final var vatPercentage = withVat ? getVatRate(previousInvoice, previousQWeek) : ZERO ; // old   VAT_RATE_2023 : ZERO;
    final var vatAmount = sum.multiply(vatPercentage.movePointLeft(2));
    final var sumWithVat = sum.add(vatAmount);
    ...
    final var advancePayment = getAdvancePayment(balanceAmount);
    final var total = sumWithVat.add(debt).subtract(advancePayment);
    final var previousWeekFee = previousInvoice.getCurrentWeekFee();
    final var previousWeekBalanceFee = previousInvoice.getPreviousWeekBalanceFee().negate();
   // final var totalWithFee = total.add(currentWeekFee).add(previousWeekBalanceFee);

    return previousWeekFee;
  }
