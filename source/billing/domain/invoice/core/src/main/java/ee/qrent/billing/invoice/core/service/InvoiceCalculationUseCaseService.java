...
import static ee.qrent.billing.transaction.api.in.utils.TransactionKindCodesConstant.*;
              ...
              driver -> {
                ...
                final var filter =
                    DriverAndPeriodAndKindCodesFilter.builder()
                        ...
                        .kindCodes(
                            Stream.of(
                                    TRANSACTION_KIND_FEE_CODE,
                                    TRANSACTION_KIND_NON_FEE_ABLE_CODE,
                                    TRANSACTION_KIND_FEE_ABLE_CODE,
                                    TRANSACTION_KIND_POSITIVE_CODE,
                                    TRANSACTION_KIND_REPAIRMENT_CODE)
                ...
                final var currentWeekFee =
                    driversTransactions.stream()
                        .filter(tx -> TRANSACTION_KIND_FEE_CODE.equals(tx.getKind()))
                ...
                final var sumOfPositiveTransactionsFromPreviousWeekNotIncludedIntoInvoice =
                    previousWeekDriversTransactions.stream()
                        ...
                        .filter(tx -> TRANSACTION_KIND_POSITIVE_CODE.equals(tx.getKind()))
                        .map(TransactionResponse::getRealAmount)
                        .reduce(BigDecimal::add)
                        .orElse(ZERO);
                ...
                final var sumOfAllNegativeTransactionsFromPreviousWeek =
                    previousWeekDriversTransactions.stream()
                        .filter(
                            tx ->
                                TRANSACTION_KIND_FEE_ABLE_CODE.equals(tx.getKind())
                                    || TRANSACTION_KIND_NON_FEE_ABLE_CODE.equals(tx.getKind())
                                    || TRANSACTION_KIND_REPAIRMENT_CODE.equals(tx.getKind())
                ...
                final var invoice =
                    Invoice.builder()
                        ...
                        .previousWeekAllPositiveTxSum(sumOfAllPositiveTransactionsFromPreviousWeek)
                        .previousWeekAllNegativeTxSum(sumOfAllNegativeTransactionsFromPreviousWeek)
                        .items(invoiceItems)
                        .build();
