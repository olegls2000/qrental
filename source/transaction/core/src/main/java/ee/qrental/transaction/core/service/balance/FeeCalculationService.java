...
public class FeeCalculationService {
  ...
  private BigDecimal getFeeAmountBasedOnPreviousWekBalance(
    ...
    final var feeAbleAmountFromPreviousWeek = balanceFromPreviousWeek.getFeeAbleAmount();
    final var feeAmountFromPreviousWeek = balanceFromPreviousWeek.getFee();

    if (feeAbleAmountFromPreviousWeek.compareTo(FEE_CALCULATION_THRESHOLD) < 0) {
      System.out.printf(
          "Debt from previous week is bigger then %s EURO (Debt: %s EUR), fee will be calculated",
          FEE_CALCULATION_THRESHOLD, feeAbleAmountFromPreviousWeek);
      final var amountFromPreviousWeekAbs = feeAbleAmountFromPreviousWeek.abs();
